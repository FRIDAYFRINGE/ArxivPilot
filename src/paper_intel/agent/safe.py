...
def _replace_uuid_citations(answer: str, chunks: list) -> str:
    ...
    prefix_to_label = {cid[:8]: label for cid, label in cid_to_label.items()}
    ...
    def _sub(m) -> str:
        ...
        if full:
            ...
            if lbl:
                return lbl
        if pre:
            lbl = prefix_to_label.get(pre) or prefix_to_label.get(pre.lower())
            if lbl:
                return lbl
        return m.group(0)
...
class AgentState:
    ...
    successful_searches: int = 0   # non-empty search results seen
    trace: list[TraceStep] = field(default_factory=list)
...
class ReactAgent:
    ...
    def run(self, question: str, history: list[dict] | None = None) -> AgentOutput:
